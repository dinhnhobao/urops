...
\section{Creation of \textit{NUSLot}}
	...
	\subsection{Cropping of spots from complete images}
		...
		\begin{figure}[H]
			\centering
			%\includegraphics[width=0.75\textwidth]{figures/daily\_cropper\_example}
	...
	\subsection{Labeling of cropped images}
		...
		\begin{figure}[H]
			\centering
			%\includegraphcs[width=0.75\textwidth]{figures/spot\_labeler\_example}
	...
	\subsection{Serialization of data}
		...
		\begin{enumerate}
			...
			set comprising of 10\% of the total number of examples at hand. The test set is then
			saved as a \texttt{.hdf5} binary file in 
		\end{enumerate}	

		Talk in detail about the manner in which things are processed. There is quite a lot to say.

Labels in ../data/labels/.
(Cropped) pictures in ../data/cropped/.
../data/hdf5/ created to store the serialized datasets.
		
mention
		
		Mention that this turned out to be far more efficient than the built-in, popular, pickle module.
		Kept running out of memory.
  
	\subsection{Culmination}
		Describe the dataset and where to get it and other practical matters.			
