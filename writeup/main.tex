...
\section{Creation of a Singaporean \textit{PKLot}: \textit{NUSLot}}
	...
	\subsection{Serialization of data}
		...
		\begin{enumerate}
			...
			training set. This is done to help the CNN's gradient descent 
			faster\cite{sub-norm}\relax. Each split is then saved
			\texttt{data/hdf5/train\_val-} 
			\texttt{idation\_set\_\{split\_number\}.hdf5}, 
			where
			\texttt{split\_number} is one of \texttt{1}, \texttt{2}, or 
			\texttt{3}.
		\end{enumerate}
		...
		classes and image types. (In fact, all aforementioned scripts can also 
		be applied to the general data-processing context.) Also to note is 
		that the third-part \texttt{h5py} module was used to serialize the data 
		into HDF5 files instead of the more commonly-used inbuilt 
		\texttt{pickle} module since the former module is far more 
		RAM-efficient for the serialization of arrays 
		\cite{hdf5-performance}\relax.
